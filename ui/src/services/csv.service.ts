...
export class ZipService {
  ...
  private async addOne(
    ...
    link: TZipLink,
    opts: Required<Pick<ZipServiceOptions, 'onEntryError' | 'fallbackToBlobReader'>> &
      Pick<ZipServiceOptions, 'compressionLevel' | 'signal' | 'onEntryProgress'>
  ): Promise<void> {
    const { signal, onEntryProgress, onEntryError, fallbackToBlobReader } = opts;
    ...
    const addViaHttpReader = async () => {
      await zipWriter.add(name, new HttpReader(link.url), {
        ...
        async onprogress(loaded, total) {
          onEntryProgress?.(name, loaded, typeof total === 'number' ? total : undefined);
        },
      });
    };
  ...
  public async getZipFileBlob(links: TZipLink[], options: ZipServiceOptions = {}): Promise<Blob> {
    const {
      compressionLevel = 0,
      zip64 = true,
      concurrency = 4,
      ...
      onOverallProgress || onEntryProgress
        ? (name: string, loaded: number, total?: number) => {
            ...
            if (onOverallProgress) {
              ...
              onOverallProgress(sumLoaded, allHaveTotals ? sumTotal : undefined);
            }
          }
        : undefined;
