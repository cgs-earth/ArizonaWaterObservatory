...
export const getId = (feature: Feature, isStringIdentifierCollection: boolean) => {
...
const highlightText = (
  ...
  while (true) {
    ...
    if (idx === -1) {
      // remainder
      if (i < text.length) {
        nodes.push(<span key={`${keyPrefix}-seg-${matchIndex}-tail`}>{text.slice(i)}</span>);
      }
      break;
    }
    if (idx > i) {
      nodes.push(<span key={`${keyPrefix}-seg-${matchIndex}`}>{text.slice(i, idx)}</span>);
    }
    nodes.push(
      <Text
        span
...
export const highlightMatches = (
  ...
  for (const [rawKey, rawVal] of Object.entries(properties)) {
    ...
    if (!keyHas) {
      ...
      const valueCmp = caseSensitive ? valueForSearch : valueForSearch.toLowerCase();
    ...
    const wrappedValue = (
      <Text key={`${key}-value`} size="xs" c="dimmed" className={styles.breakWord}>
    ...
    lines.push(
      <Group key={`kv-${key}`} component="span" gap="calc(var(--default-spacing) / 8)">
