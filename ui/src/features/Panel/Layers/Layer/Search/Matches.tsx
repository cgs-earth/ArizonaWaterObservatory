...
export const Matches: React.FC<Props> = (props) => {
  ...
  const handleDeselect = (id: Location['id']) => {
    removeLocation({ id, layerId: layer.id });
  };

  const handleViewOnMap = (event: MouseEvent<HTMLButtonElement>, feature: Feature) => {
  ...
  const selectedMatches = useMemo(() => {
    ...
    for (const location of selectedLocations) {
      ...
      if (location.properties) {
        const lines = highlightMatches(location.properties, searchTerm, lineLimit);
        if (lines.length) {
          nodes.push(
            <Box key={`sel-${id}`} component="div">
              ...
              <Group gap="calc(var(--default-spacing) / 4)" justify="space-between">
      ...
      {hasOtherMatches && (
        <Stack gap="var(--default-spacing)">
          ...
          >
            ...
            {otherMatches.length === locationLimit && <>(Top {locationLimit})</>}
          </Text>
          {otherMatches}
        </Stack>
      )}
