...
export const LayerBlock: React.FC<Props> = (props) => {
  const { locations, collection, collectionType, layer, linkLocation = null } = props;
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < chunkedLocations.length; i++) {
      ...
      if (linkLocationInChunk && page !== i + 1) {
        setPage(i + 1);
        break;
      }
    }
  ...
  return (
    ...
    >
      <Paper shadow="xs" p="var(--default-spacing)" bdrs={0} className={styles.summary}>
        <Stack gap="var(--default-spacing)">
          ...
          {collection && collectionType === CollectionType.EDR && currentChunk.length > 0 && (
            <>
              <LocationsChart layer={layer} locations={currentChunk} />
            </>
          )}
          {collection && collectionType === CollectionType.EDRGrid && currentChunk.length > 0 && (
            <>
