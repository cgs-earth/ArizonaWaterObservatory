...
export const LocationsChart: React.FC<Props> = (props) => {
  ...
  const buildLink = (locationId: string, layer: Layer): TZipLink => {
  ...
  const handleFromChange = useCallback((from: Layer['from']) => setFrom(from), [setFrom]);
  const handleToChange = useCallback((to: Layer['to']) => setTo(to), [setTo]);
  ...
  const locationIds = useMemo(() => {
    return organizedLocations.map(({ id }) => id);
  }, [organizedLocations]);
